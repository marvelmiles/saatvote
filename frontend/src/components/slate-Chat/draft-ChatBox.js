...
const Span = (props) => {
  console.log(props, "spanning...............");
  return <span className="custom-inline">{props.children}</span>;
};

const Cs = (props) => {
  console.log(props, "from cs...................");
  if (props.urlType === "image")
    return <span className="custom-inline">{props.children}</span>;
  return <figure>{props.children}</figure>;
};
const emojiElement = <Emoji emoji={{ id: "shrug" }} set="apple" size={24} />;
function ChatBox(props) {
  ...
  const blockRenderMap = Immutable.Map({
    unstyled: {
      element: "span",
      wrapper: <Span nodeType={nodeType} />,
    },
    atomic: {
      wrapper: <Cs urlType={state.urlType} />,
    },
  });
  ...
  return (
    <>
      ...
      <div style={styles.buttons}>
        <button onClick={() => setNodeType("paragraph")}>Paragraph</button>
        <button onClick={() => setNodeType("text")}>Span</button>
...
const blockStyleFn = (editorState, contentBlock) => {
  ...
  //     console.log("block style", type, entity);
  //     if (type === "atomic") {
